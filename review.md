...
- **Events sometimes didn't load after logging in from the browser.** `stores/index.js::loginUser` had a leftover `loadEvents({from, to})` call scoped to the *current week*, directly under a comment saying "Events are loaded by CalendarBody.$effect on mount — no extra call needed" — i.e. dead code that was actually harmful, not just redundant. It raced against `CalendarBody.svelte`'s own month-scoped load (triggered when the app mounts post-login); since `events.set()` replaces the whole store rather than merging, whichever call resolved last won — and the week-scoped one, firing after `loginUser`'s awaited calendar/category loads, usually won, narrowing the store to a mostly-empty single week even though the month view was rendering. Fixed by deleting the dead call (matches what `registerUser` already correctly does).
...
**Android widget, round 2 (after the user tested the above on-device):**
- **Event colors were wrong/all gray.** `WidgetDataFetcher.fetchCategoryColors` used `obj.optString("color", DEFAULT_EVENT_COLOR)` for a category's `color` field, which is nullable in the API schema. Same `org.json` gotcha fixed elsewhere this session: when the JSON value is null (not missing), `optString(key, fallback)` can return the literal string `"null"` instead of the fallback. `Color.parseColor("null")` then throws, silently caught by the accent-bar's try/catch and rendered gray. Fixed with an explicit `obj.isNull("color")` check, same pattern as the other fixes.
- **Event cards didn't match the app's month-view pill styles.** Widget only ever drew a thin accent bar + neutral-colored text for every event. Now mirrors `MonthView.svelte`'s two styles: all-day events get a solid fill in the event color + fixed dark text (no accent bar); timed events get a ~26% tinted fill (`Color.argb`, matches the CSS `color42` hex-alpha trick) + colored text + the accent bar as the "left border." `month_widget_cell.xml`'s event-row `FrameLayout`s gained small padding so the solid all-day fill reads as a proper pill.
- **Multi-day events only ever appeared on their first day.** `buildEventsByDate` computed one `WidgetEvent` per occurrence and filed it under only the occurrence's start date. Widget can't draw a connected banner across grid cells the way the app does (that's a much bigger rendering model — out of scope), but it can at least not lose the event entirely: it now computes each event's span in days and files a `WidgetEvent` under every day it covers (clipped to the visible grid), for both plain and recurring events.
- One XML lesson: an `assembleDebug` failure ("not well-formed... invalid token") turned out to be a `--` inside an XML *comment* (`.ev-pill--allday`) — XML comments cannot contain a literal double-hyphen anywhere in their body, not just at the boundaries. Watch for this specifically when a comment describes CSS class names with double-dash BEM-style modifiers.
- Still not verified on-device (see above) — these are believed-correct from re-reading the code + successful builds, not confirmed by the user yet.
