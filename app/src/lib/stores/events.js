...
export const visibleEvents = derived(
  ...
  ([$events, $cals, $cats]) => {
    ...
    return $events.filter(e => {
      ...
      // Colour precedence: the event's own override, then its category,
      // then the default accent. The override comes first — that is the
      // point of it. (This used to be the other way round, so a per-event
      // colour was accepted by the form and then never shown.)
      //
      // `color` here is the *resolved* colour for painting. The unresolved
      // override stays available as `colorOverride` so the editor can tell
      // "inheriting" from "deliberately set to the category's colour".
      const cat = e.category_id ? $cats.find(c => c.id === e.category_id) : null;
      return {
        ...e,
        colorOverride: e.color ?? null,
        color: e.color ?? cat?.color ?? '#b8c9f4',
      };
    });
  }
);
