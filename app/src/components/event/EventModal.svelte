<script>
  ...
  import {
    canExportToSystemCalendar, exportToSystemCalendar,
  } from '../../lib/services/systemCalendar.service.js';
  ...
  $effect(() => { canExportToSystemCalendar().then(v => { canExport = v; }); });

  async function onAddToDeviceCalendar() {
    try {
      await exportToSystemCalendar(ev);
      // Deliberately not "Added": the system editor gives no callback, so
      // all we know is that it opened. The user still has to save there.
      showToast('Opening your calendar app…', 'info');
    } catch (e) {
      showToast(
        e?.message === 'NO_CALENDAR_APP'
      ...
      {#if canEdit}
        <div class="actions">
          ...
          {#if canExport}
            <button
              class="btn-duplicate"
              onclick={onAddToDeviceCalendar}
              title="Copy this event to your device's calendar"
            >📅</button>
          {/if}
