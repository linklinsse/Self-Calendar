<script>
  ...
  /** Back to inheriting the category's colour (persisted as null). */
  ...
  function handleSave() {
    ...
    saveEvent({
      ...
      // Only persist a colour when it actually differs from the category's.
      // Storing one that merely matches would silently pin the event: a
      // later recolour of the category would leave this event behind on the
      // old colour, with nothing in the UI explaining why.
      color: colorOverridden ? form.color : null,
    });
  }
