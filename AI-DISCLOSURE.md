...
| Measure | Share |
|---|---|
| Lines dating from before the first commit that mentions Claude (2026-03-31) | 2% |
| Lines written in the 2026-07/08 review-and-fix sessions | 30% |
| Lines from the maintainer's own commits | 69% |

That last figure is the reason the headline number is an estimate rather than
...
Being honest about the method means being honest about what it cost:

- **Some of this code has never run on real hardware.** The Android widget,
  the device-calendar export and the deep links were written and reasoned
  about but not executed on a device.
- **AI-written code was the source of real bugs**, several found only because
  a later session went looking. A weekly recurrence bug survived two
  independent reviews and was caught by a conformance test, not by reading.
- **The comments explain intent unusually thoroughly**, which is a genuine
  benefit of the method and is why several non-obvious decisions in this
  codebase are documented at the point they could be undone by accident.
